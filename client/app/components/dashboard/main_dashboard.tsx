...
export const MainDashboard = () => {
    ...
    const tabButtonClass = (tab: string) =>
        `text-white text-sm md:text-base py-[0.219rem] px-3 rounded-[1.563rem] font-jost font-medium cursor-pointer text-center transition-all duration-150 ease-in-out ${
            activeTab === tab ? "bg-linear-[90deg,#384ef4,#b060ed]" : "hover:bg-linear-[90deg,#384ef4,#b060ed]"
        }`;

    const filterButtonClass = (level: "beginner" | "advance") =>
        `p-1.5 rounded-[1.563rem] min-w-[6.75rem] cursor-pointer text-sm md:text-base font-medium ${
            eduLevel === level ? "bg-black text-white" : "bg-transparent text-black"
        }`;
